...
public class RestoreMonsterPanel extends Panel {
    public RestoreMonsterPanel(HomeGUI homeGUI) {
        ...
        if (Javamon.getHomebaseMonsters().length == 0) {
    ...
    private JCheckBox[] getMonsterColumns() {
        ...
        int monsterCount = Javamon.getHomebaseMonsters().length;
        JCheckBox[] monsterColumns = new JCheckBox[monsterCount];

        for (int i = 0; i < monsterCount; i++) {
            Monster monster = Javamon.getHomebaseMonster(i);
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                for (int i = 0; i < count; i++) {
                    for (int j = 0; j < monsterCount; j++) {
                        Monster dummyMonster = Javamon.getHomebaseMonster(j);
